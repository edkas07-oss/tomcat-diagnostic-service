...
export function createRequestHandler(options) {
  return async (request, response) => {
    ...
    if (request.method === "GET" && request.url === "/metrics") { response.writeHead(200, { "content-type": "text/plain; version=0.0.4; charset=utf-8" }); return response.end(options.metricsText()); }
    ...
    for await (const chunk of request) { size += chunk.length; if (size > (options.requestLimitBytes ?? 256 * 1024)) return json(response, 413, { error: "payload_too_large" }); chunks.push(chunk); }
